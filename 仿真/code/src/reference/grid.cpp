...
static std::vector<ReferenceStep> solve(const Observations& obs,double bound,double spacing,bool midpoint) {
  ...
  Vector predicted(n),drift(n),log_weights(n);
  for(std::size_t t=0;t<obs.values.size();++t) {
    const int k=static_cast<int>(t)+1;
    for(int i=0;i<n;++i)drift[i]=gordon_drift(k,grid[i]);
    for(int j=0;j<n;++j) {
      // 概率质量形式求积, 常数正态归一化因子在 Bayes 更新时约掉。
      predicted[j]=((-0.05*(drift.array()-grid[j]).square()).exp()*weights.array()).sum();
      const double residual=obs.values[t]-grid[j]*grid[j]/20;
      log_weights[j]=std::log(predicted[j])+std::log(quadrature[j])-0.5*residual*residual;
    }
...
ReferenceRecord grid_reference(const PublicDataset& data,const Observations& obs,const GridConfig& config) {
  ...
  auto coarse=solve(obs,config.bound,config.spacing,true);
  auto middle=solve(obs,config.bound,config.spacing/2,true);
  auto fine=solve(obs,config.bound,config.spacing/4,true);
  auto expanded=solve(obs,config.bound*1.25,config.spacing/4,true);
  auto cross=solve(obs,config.bound*1.25,config.spacing/4,false);
