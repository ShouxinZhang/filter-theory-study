...
void generate_dataset(const fs::path& directory,const std::string& name,const std::string& variant,
  ...
  auto model=make_environment(name);
  ...
  for(int id=0;id<count;++id) {
    ...
    Vector state(model->dimension()),next(model->dimension());
    ...
    for(int k=1;k<=model->steps();++k) {
      if(k>model->initial_time()) { model->propagate(k,state,next,process);state.swap(next); }
