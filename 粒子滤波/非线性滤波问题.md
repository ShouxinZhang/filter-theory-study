...
给定预测经验测度 $\hat{\mathbb{P}}_{N;X_k}^-=\sum_{i=1}^N w_k^{-,(i)}\delta_{x_k^{-,(i)}}$，其中权重非负且和为 $1$。若
$$
0<\sum_{i=1}^N w_k^{-,(i)}p_{Y_k\mid X_k}(y_k\mid x_k^{-,(i)})<\infty,
$$
则用观测似然对该测度加权并归一化后，所得测度仍支撑在原粒子上，且满足
$$
\begin{aligned}
&\hat{\mathbb{P}}_{N;X_k}^+
=\sum_{i=1}^N w_k^{+,(i)}\delta_{x_k^{-,(i)}},\\
&w_k^{+,(i)}
=\frac{w_k^{-,(i)}p_{Y_k\mid X_k}(y_k\mid x_k^{-,(i)})}
{\displaystyle\sum_{j=1}^N w_k^{-,(j)}p_{Y_k\mid X_k}(y_k\mid x_k^{-,(j)})}.
\end{aligned}
$$

#### 证明

先将 Bayes 更新写成测度形式。对任意 $A\in\mathcal{B}(\mathcal{X})$，用似然对预测测度在 $A$ 上的质量加权，再除以整个状态空间上的加权总质量：
$$
\hat{\mathbb{P}}_{N;X_k}^+(A)
:=\frac{\displaystyle\int_A p_{Y_k\mid X_k}(y_k\mid x)\,\mathrm{d}\hat{\mathbb{P}}_{N;X_k}^-(x)}
{\displaystyle\int_{\mathcal{X}}p_{Y_k\mid X_k}(y_k\mid x)\,\mathrm{d}\hat{\mathbb{P}}_{N;X_k}^-(x)}.
$$
该式以测度积分表达归一化加权，因此可以直接用于离散的经验测度。

Dirac 测度满足 $\int\varphi(x)\,\mathrm{d}\delta_a(x)=\varphi(a)$。将预测经验测度的有限和代入分子，并使用积分的线性性，积分就变成各粒子处的似然乘以权重与集合指示函数：
$$
\begin{aligned}
&\int_A p_{Y_k\mid X_k}(y_k\mid x)\,\mathrm{d}\hat{\mathbb{P}}_{N;X_k}^-(x)\\
= &\sum_{i=1}^N w_k^{-,(i)}p_{Y_k\mid X_k}(y_k\mid x_k^{-,(i)})\mathbb{I}_A(x_k^{-,(i)}).
\end{aligned}
$$
取 $A=\mathcal{X}$，所有指示函数均为 $1$，便得到分母的加权似然和。按定理假设可以作除法，代回即得
$$
\begin{aligned}
&\hat{\mathbb{P}}_{N;X_k}^+(A)\\
= &\sum_{i=1}^N
...
\end{aligned}
$$
这对任意可测集合 $A$ 成立，因此给出定理中的测度等式。更新后的权重非负且和为 $1$，故所得测度是概率测度。证毕。

这里的等式是对给定预测经验测度进行更新的精确结果，它与真实后验之间的关系仍是粒子近似。
