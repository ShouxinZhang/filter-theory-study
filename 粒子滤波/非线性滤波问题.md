...
位置变换中的粒子矩阵统一按列拼接，每列是一个粒子：
$$
\begin{aligned}
&X_k:=x_k^{(1..N)}:=[x_k^{(1)},\ldots,x_k^{(N)}]\in\mathbb R^{n_x\times N},\\
&X_k^\pm:=x_k^{\pm,(1..N)}\in\mathbb R^{n_x\times N},\;x_k^{\pm,(i)}\in\mathbb R^{n_x\times1}.
\end{aligned}
$$
这里 $X_k$ 表示粒子矩阵，概率分布下标中的 $X_k$ 仍表示状态随机变量，由上下文区分。
用 $\bm\Pi_k$ 表示联合概率矩阵，$\bm T_k\in\mathbb R^{N\times N}$ 表示位置变换矩阵，$\bm n_k$ 表示各粒子的复制次数。$(\bm T_k)_{ij}$ 表示源粒子 $i$ 对目标粒子 $j$ 的贡献系数，即 $x_k^{+,(j)}=\sum_i(\bm T_k)_{ij}x_k^{-,(i)}$。经典重采样中，令 $A_j$ 为目标粒子 $j$ 抽取的源索引，取 $(\bm T_k)_{ij}=\mathbb I_{\{A_j=i\}}$，于是：
$$
\begin{aligned}
...
将新粒子的权重统一设为 $1/N$，在当前转移先验提议下等权进入下一轮预测，即 $w_{k+1}^{-,(j)}=1/N$。原来由权重表达的质量差异，就改由复制次数表达。
这样，$\bm T_k$ 的第 $j$ 列记录新粒子 $j$ 复制了谁，每列和为 $1$；第 $i$ 行的和是源粒子 $i$ 的复制次数 $n_k^{(i)}$。单次重采样会产生随机误差；固定原粒子与权重后，对重采样随机性取平均，所得经验测度等于原加权测度。
