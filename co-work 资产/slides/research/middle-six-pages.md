...
- $X_0$ 与全部过程噪声，观测噪声相互独立。
- 状态满足一阶 Markov 性，观测条件独立。
- 密度存在，似然可算，归一化项有限且为正。

目标是后验分布 $p(x_k\mid y_{1:k})$，后验均值只是其中一个统计量。
...
&\hspace{1em}\cdot p(x_{k-1}\mid y_{1:k-1})\,\mathrm d x_{k-1}.
\end{aligned}
$$
...
dragPos:
  ...
  particle-update: 502,100,436,226,0
...
&\hat{\mathbb P}_{N;X_k}^-=\frac1N\sum_i\delta_{x_k^{-,(i)}}.
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb P(A_j=i\mid\mathbf w_k^+)=w_k^{+,(i)},\\
&x_k^{+,(j)}=x_k^{-,(A_j)},\;w_k^{(j)}=1/N.
\end{aligned}
$$

基本版本独立抽取 $A_1,\ldots,A_N$，高权重粒子更易被复制。仿真实现采用系统重采样。
...
$$
\mathbb E[n_i/N\mid\mathbf w_k^+]=w_k^{+,(i)}.
$$

重采样恢复等权表示，但一次实现会引入随机误差并产生重复粒子。缺测步只预测，不做似然更新。
...
源质量 $\mathbf a$，目标质量 $\mathbf b$。

$$
\begin{aligned}
&\Pi_{ij}=\mathbb P(X=x_i,Y=y_j),\\
&\mathbf\Pi\mathbf1_n=\mathbf a,\;
\mathbf\Pi^{\mathsf T}\mathbf1_m=\mathbf b,\\
&U(\mathbf a,\mathbf b)=\{\mathbf\Pi\geq0:\text{满足上述边缘}\},\\
&\min_{\mathbf\Pi\in U(\mathbf a,\mathbf b)}
...
$$
\min_{\mathbf\Pi\in U(\mathbf a,\mathbf b)}
\left\{\langle\mathbf\Pi,\mathbf C\rangle_F
+\varepsilon\sum_{i,j}\Pi_{ij}(\log\Pi_{ij}-1)\right\}.
$$

固定边缘时，负熵与 $D_{\mathrm{KL}}(\mathbf\Pi\|\mathbf a\mathbf b^{\mathsf T})$ 仅差常数。

$$
\begin{aligned}
&K_{ij}=\exp(-C_{ij}/\varepsilon),\\
&\mathbf\Pi=\operatorname{diag}(\mathbf u)\mathbf K\operatorname{diag}(\mathbf v).
\end{aligned}
$$
...
从 $\mathbf v^{(0)}=\mathbf1$ 开始，逐元素相除：

$$
\begin{aligned}
&\mathbf u^{(\ell+1)}=\mathbf a\oslash(\mathbf K\mathbf v^{(\ell)}),\\
&\mathbf v^{(\ell+1)}=\mathbf b\oslash(\mathbf K^{\mathsf T}\mathbf u^{(\ell+1)}).
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbf a=\mathbf w_k^+,\;\mathbf b=\mathbf1_N/N,\\
&C_{ij}=\|x_k^{-,(i)}-x_k^{-,(j)}\|^2,\\
&\mathbf T_k=N\mathbf\Pi_k^{\mathsf T},\\
...
分布与协方差未必守恒。原始 ETPF 求线性规划；仿真用 $\varepsilon>0$ 的正则化版本。
...
BPF 与 ETPF 均呈双峰；EKF 为单个高斯峰。集合方法分别用 Scott 带宽，峰高不能直接作为精度判据。

多峰不保证均值误差更低。采用稳定化 Sinkhorn 与边缘修正。
...
ETPF 误差非单调，未见明确精度收益。结论限于 8 条轨迹及当前参数。
